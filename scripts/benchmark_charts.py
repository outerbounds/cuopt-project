...
for name, results in series.items():
    ...
    times = [r['solve_time'] for r in results]
    ax1.plot(regions, times, marker=markers[name], color=colors[name],
             linewidth=2, markersize=7, label=name)

    # Label the last point with solve time
    ax1.annotate(f'{times[-1]:.1f}s', (regions[-1], times[-1]),
...
for name, results in cpu_solvers.items():
    regions = [r['n_regions'] for r in results]
    speedups = [cpu['solve_time'] / gpu['solve_time']
                for cpu, gpu in zip(results, cuopt)]
    ax2.plot(regions, speedups, marker=markers[name], color=colors[name],
             linewidth=2, markersize=7, label=f'vs {name}')

    # Label last point
    ax2.annotate(f'{speedups[-1]:.0f}x', (regions[-1], speedups[-1]),
                 textcoords='offset points', xytext=(8, 0),
