...
if result['solution']:
    ...
    print(f"  {'Total':>8}: {sol.sum():6.1f} MW  (${(sol * cost_per_mwh).sum():,.0f}/hr)")
...
try:
    ...
    sol = np.array(result['solution'])
    source_colors = {'Solar': '#f4a41a', 'Wind': '#4aa3df', 'Gas': '#7f8c8d', 'Nuclear': '#9b59b6'}

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5.5), gridspec_kw={'width_ratios': [1, 1.2]})
    ...
    ax1.barh(y_pos, caps, height=0.5, color='#ecf0f1', edgecolor='#bdc3c7',
             linewidth=1, label='Capacity')
    # Allocated (filled)
    bars = ax1.barh(y_pos, alloc, height=0.5,
    ...
    for i, (name, actual, limit, sense) in enumerate(constraint_info):
        ...
        ax2.text(max_val + max_val * 0.03, i, label, va='center', fontsize=8.5,
