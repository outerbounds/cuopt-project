...
class PortfolioOptimizationFlow(ProjectFlow):
    ...
    def start(self):
        ...
        print(
            f"Selected {self.n_assets_actual} assets with {self.n_observations} return observations"
        )
    ...
    def generate_scenarios(self):
        ...
        simulated_scenarios = np.random.multivariate_normal(mean, cov, size=n_simulated)
    ...
    def optimize(self):
        ...
        print(
            f"Problem size: ~{self.n_assets_actual * len(self.scenarios):,} constraints"
        )
        ...
        for lambda_risk in risk_aversions:
            ...
            for i in range(self.n_assets_actual):
                w_vars.append(
                    problem.addVariable(lb=0.0, ub=1.0, vtype=CONTINUOUS, name=f"w_{i}")
                )
            ...
            for s in range(n_scenarios):
                u_vars.append(
                    problem.addVariable(lb=0.0, ub=1e6, vtype=CONTINUOUS, name=f"u_{s}")
                )
            ...
            # CVaR = t + (1/alpha) * E[u]
            obj_expr = sum(
                self.expected_returns[i] * w_vars[i]
                for i in range(self.n_assets_actual)
            )
            ...
            self.frontier_results.append(
                {
                    ...
                    "var": float(-var_pct),
                    "sharpe": (
                        float(portfolio_return / portfolio_vol)
                        if portfolio_vol > 0
                        else 0
                    ),
            ...
            print(
                f"  λ={lambda_risk:5.1f}: Return={portfolio_return:6.2%}, "
    ...
    def build_card(self):
        ...
        if holdings_rows:
            current.card.append(
                Table(holdings_rows, headers=["Ticker", "Weight", "Expected Return"])
            )
