...
class LPBenchmarkFlow(ProjectFlow):
    ...
    def start(self):
        ...
        print(
            f"{'Size':>6} {'Regions':>10} {'Variables':>12} "
        ...
        for i, s in enumerate(self.size_list):
            ...
            print(
                f"{label:>6} {s:>10,} {s * 4:>12,} " f"{2 * s + 9:>14,} {32 * s:>12,}"
            )
        self.next(self.fan_cuopt, self.fan_ortools, self.fan_simplex, self.fan_ipm)
    ...
    def solve_cuopt(self):
        ...
        print(
            f"cuOpt {n_regions:,} regions: {self.result['status']}, "
    ...
    def solve_ortools(self):
        ...
        print(
            f"OR-Tools {n_regions:,} regions: {result['status']}, "
    ...
    def solve_simplex(self):
        ...
        print(
            f"Simplex {n_regions:,} regions: {result['status']}, "
            f"profit=${result.get('total_profit') or 0:,.0f}, "
            f"build={result['build_time']:.4f}s, solve={result['solve_time']:.4f}s"
        )
    ...
    def solve_ipm(self):
        ...
        print(
            f"IPM {n_regions:,} regions: {result['status']}, "
            f"profit=${result.get('total_profit') or 0:,.0f}, "
            f"build={result['build_time']:.4f}s, solve={result['solve_time']:.4f}s"
        )
    ...
    def compare(self, inputs):
        ...
        all_series = [
            self.cuopt_results,
            self.ortools_results,
            self.simplex_results,
            self.ipm_results,
        ]
        ...
        solve_spec = {
            ...
            "encoding": {
                ...
                "tooltip": [
                    ...
                    {"field": "time", "title": "Solve Time (s)", "format": ".4f"},
                ],
            },
        }
        ...
        speedup_spec = {
            ...
            "encoding": {
                ...
                "tooltip": [
                    ...
                    {"field": "regions", "title": "Regions", "format": ","},
                    {"field": "speedup", "title": "Speedup", "format": ".1f"},
                ],
            },
        }
        ...
        gpu_vs_cpu_pdlp = largest_ortools["solve_time"] / largest_cuopt["solve_time"]
        findings.append(
        ...
        best_classical = min(largest_simplex["solve_time"], largest_ipm["solve_time"])
        best_name = (
            "IPM"
            if largest_ipm["solve_time"] < largest_simplex["solve_time"]
            else "Simplex"
        )
        ...
        # IPM vs Simplex
        ipm_vs_simplex = largest_simplex["solve_time"] / largest_ipm["solve_time"]
        if ipm_vs_simplex > 1.1:
            findings.append(
                f"- IPM is **{ipm_vs_simplex:.1f}x** faster than simplex " f"at scale"
            )
    ...
    def end(self):
        ...
        print(
            f"{'Regions':>10} {'Variables':>12} {'cuOpt PDLP':>14} "
            f"{'OR-Tools PDLP':>14} {'Simplex':>14} {'IPM':>14} "
            f"{'Winner':>10}"
        )
        for cu, ort, sx, ipm in zip(
            self.cuopt_results,
            self.ortools_results,
            self.simplex_results,
            self.ipm_results,
