...
class CuOptLPFlow(ProjectFlow):
    ...
    def start(self):
        ...
        n_cons = (
            n_reg * 2 + 4 + 5
        )  # per-region (land + water) + market caps + labor pools
    ...
    def solve_scenario(self):
        ...
        obj = sum(
            float(profit[r, c]) * x[r][c] for r in range(n_reg) for c in range(n_crops)
        )
        ...
        for k in range(n_pools):
            ...
            problem.addConstraint(
                pool_expr <= float(pool_capacity[k]), name=f"labor_pool_{k}"
            )
        ...
        print(
            f"Scenario {price_mult:.0%}: {status}, profit=${profit_val:,.0f}, "
    ...
    def join(self, inputs):
        ...
        self.results = sorted([i.result for i in inputs], key=lambda r: r["scenario"])
        feasible = [r for r in self.results if r.get("total_profit") is not None]
        baseline = next(
            (r for r in feasible if r["scenario"] == 1.0),
        ...
        if baseline:
            ...
            rows = [
                [crop, f"{acres:,.0f}"]
                for crop, acres in baseline["crop_totals"].items()
            ]
            current.card.append(
                Table(rows, headers=["Crop", "Total Acres (all regions)"])
            )

            current.card.append(
                Markdown(f"**Total Profit:** ${baseline['total_profit']:,.0f}")
            )
            current.card.append(
                Markdown(f"**Solve Time:** {baseline['solve_time']:.4f}s")
            )
            current.card.append(
                Markdown(f"**Build Time:** {baseline['build_time']:.2f}s")
            )
            ...
            if baseline.get("pool_utilizations"):
                ...
                current.card.append(
                    Table(pool_rows, headers=["Labor Pool", "Utilization"])
                )
                current.card.append(
                    Markdown(
                        f"**Avg Pool Utilization:** {baseline['avg_pool_util']:.1%}"
                    )
                )
        ...
        if len(feasible) > 1:
            ...
            for r in feasible:
                delta = (
                    (
                        (r["total_profit"] - baseline["total_profit"])
            ...
            current.card.append(
                Table(
                    scenario_rows,
                    headers=["Price", "Profit", "vs Baseline", "Solve Time"],
                )
            )
    ...
    def end(self):
        ...
        if baseline and baseline.get("total_profit"):
            print(
                f"Optimal profit at baseline prices: ${baseline['total_profit']:,.2f}"
            )
