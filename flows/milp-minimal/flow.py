...
class CuOptMILPFlow(ProjectFlow):
    ...
    def _build_problem(self, name):
        ...
        y = [
            problem.addVariable(lb=0, ub=1, vtype=INTEGER, name=f"y_{j}")
            for j in range(nf)
        ]
        ...
        x = [
            [
                problem.addVariable(lb=0.0, ub=1.0, vtype=CONTINUOUS, name=f"x_{i}_{j}")
                for j in range(nf)
            ]
            for i in range(nc)
        ]
    ...
    def start(self):
        ...
        self.capacity = [
            random.uniform(avg_cap * 0.5, avg_cap * 1.5) for _ in range(nf)
        ]
        ...
        print(f"Variables: {nf} binary + {nf * nc:,} continuous = " f"{nf + nf * nc:,}")
    ...
    def solve_heuristic(self):
        ...
        print(
            f"Heuristic: {h['status']}, cost=${cost:,.0f}, "
    ...
    def compare(self, inputs):
        ...
        rows = [
            ...
            [
                "Facilities Opened",
                str(h.get("n_open", "N/A")),
                str(e.get("n_open", "N/A")),
            ],
            ["Solve Time", f"{h['solve_time']:.2f}s", f"{e['solve_time']:.2f}s"],
        ]
        current.card.append(
            Table(rows, headers=["Metric", "GPU Heuristics", "Full B&B"])
        )
        ...
        if h.get("total_cost") and e.get("total_cost"):
            ...
            if gap > 0.01:
                ...
                current.card.append(
                    Markdown(f"**Result:** Both methods found the same optimum.")
                )
        ...
        map_spec = {
            ...
            "encoding": {
                ...
                "color": {
                    ...
                    "title": "Decision",
                    "scale": {
                        "domain": ["Both Open", "Heuristic Only", "B&B Only", "Closed"],
                        "range": ["#4C9878", "#E6786C", "#5B8DBE", "#cccccc"],
                    },
                },
                "shape": {
                    "field": "type",
                    ...
                    "scale": {
                        "domain": ["Both Open", "Heuristic Only", "B&B Only", "Closed"],
                        "range": ["diamond", "triangle-up", "triangle-down", "circle"],
                    },
                },
            },
        }
